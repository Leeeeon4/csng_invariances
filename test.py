#%%
import torch
from csng_invariances.models.encoding import HSMModel
from csng_invariances.data.datasets import Antolik2016Dataset
...
train_dataloader = DataLoader(
    Antolik2016Dataset(dataset_type="training"), batch_size=32, shuffle=True
)
validation_dataloader = DataLoader(
    Antolik2016Dataset(dataset_type="validation"), batch_size=32, shuffle=True
)

one_image, one_response = next(iter(train_dataloader))
batch_size, channels, height, width = one_image.shape
batch_size, num_responses = one_response.shape

encoding_model = HSMModel(images_size=one_image.shape, num_neurons=num_responses)

print(encoding_model)

loss_fn = torch.nn.PoissonNLLLoss()
optimizer = torch.optim.SGD(encoding_model.parameters(), lr=learning_rate)

for t in track(range(epochs)):
    loss = train_loop(train_dataloader, encoding_model, loss_fn, optimizer)
    if epochs % 10 == 0:
        print(loss)
