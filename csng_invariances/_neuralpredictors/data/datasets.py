...
class FileTreeDataset(StaticSet):
        ...
        *data_keys,
        ...
        Args:
            ...
            *data_keys:  data items to be extraced (must be subdirectories of root/data)
            ...
            image_preprocessing: function of image preprocessing steps
            response_preprocessing: function of response preprocessing steps
        """
        super().__init__(*data_keys, transforms=transforms)
    ...
    def __getitem__(self, item):
        ...
        def preprocess(data, image_preprocessing, response_preprocessing):
            """Apply preprocessing.

            image_preprocess is applied on images. response_preprocess is applied
            on responses.

            Args:
                data (Array): datapoint to apply preprocessing to.
                image_preprocessing (function): Function of image preprocessing steps
                response_preprocessing (function): Function of response preprocessing steps

            Returns:
                Array: preprocesses datapoint
            """
            if len(data.shape) == 3 and image_preprocessing is not None:
                value = image_preprocessing(data)
            elif len(data.shape) == 1 and response_preprocessing is not None:
                value = response_preprocessing(data)
            else:
                value = data
            return value
        ...
        for data_key in self.data_keys:
            if item in self._cache[data_key]:
                value = preprocess(
                    data=self._cache[data_key][item],
                    image_preprocessing=self.image_preprocessing,
                    response_preprocessing=self.response_preprocessing,
                )
                ret.append(value)
            else:
                datapath = self.resolve_data_path(data_key)
                ...
                self._cache[data_key][item] = value
                ret.append(value)
