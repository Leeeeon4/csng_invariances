...
from staticnet_experiments.configs import (
    CoreConfig,
    ReadoutConfig,
    ShifterConfig,
    ModulatorConfig,
...
dj.config["external-data"] = dict(protocol="file", location="/external/")
...
def best_model(m, aggr=None):
    ...
    return (
        target * aggr_target.aggr(target, best_corr="max(val_corr)")
        & "best_corr = val_corr"
    )
...
def best_model_new(m, aggr=None):
    ...
    :return: best network model for each combination of StaticMultiDataset and DataConfig
    """
    corrs_per_model = dj.U("group_id", "net_hash").aggr(
        m, best_corr="MAX(val_corr)", avg_corr="AVG(val_corr)"
    )  # per model but across seeds
    ...
    best_model = aggr_target.aggr(
        target, best_avg="max(avg_corr)"
    )  # best model for each StaticMultiDataset * DataConfig combo
...
class ProcessedMonitorCalibration(dj.Computed):
    ...
    class GammaFit(dj.Part):
        ...
        def get_fs(self, key=None):
            if key is None:
                key = {}
            amplitude, gamma, offset = (self & key).fetch1(
                "amplitude", "gamma", "offset"
            )
        ...
        def get_interp(self, key=None):
            if key is None:
                key = {}
            px, lum = (experiment.MonitorCalibration & (self & key).proj()).fetch1(
                "pixel_value", "luminance"
            )
    ...
    def make(self, key):
        ...
        pixel_value, luminance = (experiment.MonitorCalibration & key).fetch1(
            "pixel_value", "luminance"
        )
...
class ClosestCalibration(dj.Computed):
    ...
    def make(self, key):
        ...
        calib = (
            ...
            * experiment.Session
            * experiment.Scan()
            * ProcessedMonitorCalibration.GammaFit()
        )
        calib_map = dict(
            calib_animal_id="animal_id",
            calib_session="session",
            calib_scan_idx="scan_idx",
        )
        rel = target_scan * calib.proj(**calib_map, calib_ts="scan_ts")
        delta = rel.proj(
            delta_time="ABS(TIMESTAMPDIFF(SECOND, scan_ts, calib_ts))/3600"
        )
...
class TargetModel(dj.Computed):
    ...
    def fill(self, group_id, dryrun=False):
        ...
        ro_types = [
            ReadoutConfig.ModifiedSpatialTransformerPyramid2d & "downsample=1",
        ...
        data_types = [
            ...
            & dict(stimulus_type="stimulus.Frame", exclude="images,responses")
        ]
        ...
        for cond in conds:
            keys = best_model(
                Model & (NetworkConfig.CorePlusReadout & cond) & restr
            ).fetch("KEY")
            if len(keys):
...
class TargetDataset(dj.Computed):
    ...
    def make(self, key):
        ...
        for readout_key in trainsets.keys():
            ...
            member_key = (
                StaticMultiDataset.Member() & key & 'name="{}"'.format(readout_key)
            ).fetch1("KEY")
            ...
            for neuron_id, unit_id, brain_area, layer in zip(
                count(), unit_ids, areas, layers
            ):
                tuple = dict(ro_tuple)
...
class OracleRankedUnit(dj.Computed):
    ...
    def make(self, key):
        keys, corr = (TargetDataset.Unit * Oracle.UnitScores & key).fetch(
            "KEY", "pearson", order_by="pearson DESC"
        )
        N = len(keys)
        unit_keys = []
...
class ModelGroup(dj.Computed):
    ...
    def make(self, key):
        best_cnn = best_model(
            ...
            & key
            & (
                NetworkConfig.CorePlusReadout * TargetModel
                & key
                & [CoreConfig.GaussianLaplace]
            )
        )
        best_lin = best_model(
            Model
            ...
            & (
                NetworkConfig.CorePlusReadout * TargetModel
                & key
                & [CoreConfig.StackedLinearGaussianLaplace]
            )
        )
...
class HighUnitSelection(dj.Computed):
    ...
    def make(self, key):
        ...
        all_data = (
            r
            * cnn_scores.proj(cnn_hash="net_hash", cnn_corr="pearson", cnn_seed="seed")
            * lin_scores.proj(lin_hash="net_hash", lin_corr="pearson", lin_seed="seed")
            * meso.ScanSet.UnitInfo()
        )

        keys, oracle_corr, cnn_corr, lin_corr, posx, posy, posz = all_data.fetch(
            "KEY", "pearson", "cnn_corr", "lin_corr", "um_x", "um_y", "um_z"
        )
        keys = np.array(keys)
        ...
        print(
            "Overlap = %.2f %%"
            % (100 * len(pos) / (cnn_selection.sum() + lin_selection.sum() - len(pos)))
        )
        ...
        ), "Overlap still remains. Check the algorithm"
...
class CorrectedHighUnitSelection(dj.Computed):
    ...
    def make(self, key):
        ...
        all_data = (
            all_data
            & "ABS(px_y - px_height / 2) < px_height / 2 - {} * px_height / um_height".format(
                dist_thr
            )
            & "ABS(px_x - px_width / 2) < px_width / 2 - {} * px_height / um_height".format(
                dist_thr
            )
        )
        ...
        print(
            "Excluded {} / {} neurons lying within {} of edge".format(
                n_before - n_after, n_before, dist_thr
            )
        )
        ...
        good_keys = keys[sorted_ordering]
        xyz_dist = np.sqrt(
            (px[:, None] - px) ** 2 + (py[:, None] - py) ** 2 + (pz[:, None] - pz) ** 2
        )
        nearby = xyz_dist < 20
...
class NonSpatialHighUnitSelection(dj.Computed):
    ...
    def make(self, key):
        ...
        keys, oracle_corr, cnn_corr, lin_corr = all_data.fetch(
            "KEY", "pearson", "cnn_corr", "lin_corr"
        )
        keys = np.array(keys)
...
def contrast_tuning(
    model, img, bias, scale, min_contrast=0.01, n=1000, linear=True, use_max_lim=False
):
    mu = img.mean()
    delta = img - img.mean()
    vmax = delta.max()
    ...
    lim_contrast = (
        255 / (vmax - vmin) * base_contrast
    )  # maximum possible reachable contrast without clipping
    ...
    def run(x):
        with torch.no_grad():
            img = torch.Tensor(
                process(x[..., None], mu=bias, sigma=scale)[None, ...]
            ).cuda()
            result = model(img)
        return result
    ...
    if clipped and force:
        ...
        for g in gains:
            ...
            if offset < 0:  # pixel values needs to be raised
                ...
                va = ((mask * (img - img_min)).sum(axis=(1, 2)) + v * nhigh) / (
                    nlow + nhigh
                )
                pos = np.argmin(np.abs(va - offset))
                actual_offset = v[pos]
...
def adjust_contrast_with_mask(
    ...
    if clipped and force:
        ...
        for g in tqdm(gains, disable=(not verbose)):
            ...
            nlow = (mask * img_mask).sum(
                axis=(1, 2)
            )  # effective number of pixels that are closer to the bound than v
            nhigh = img_mask.sum() - nlow

            # calculate the actual shift in mean that can be achieved by shifting all pixels by v
            # then clipping
            va = (
                (mask * img_mask * sign * (edge - img)).sum(axis=(1, 2))
                + (v[:, None, None] * img_mask * (1 - mask)).sum(axis=(1, 2))
            ) / (nlow + nhigh)
...
def adjust_img_stats(
    ...
    if sigma_i < 1e-8:
        ...
        max_gain = min(
            max(img_max - mu, mu - img_min) / np.min(np.abs(delta)[mask > 0]), max_gain
        )
        min_gain = min_gain
    ...
    if pos is None:
        ...
        def adj_model(x):
            ...
            for m in models:
                resp = (
                    resp
                    + m(x, readout_key, eye_pos=mu_eye, behavior=mu_beh)[:, neuron_id]
                )
            return resp / len(models)
        ...
        def adj_model(x):
            return models[pos](x, readout_key, eye_pos=mu_eye, behavior=mu_beh)[
                :, neuron_id
            ]
...
class MultiModelEvaluation(dj.Computed):
    ...
    def make(self, key):
        testsets, testloaders = DataConfig().load_data(
            key, tier="test", cuda=True, batch_size=30
        )
        ...
        for readout_key, testloader in testloaders.items():
            ...
            unit_scores.extend(
                [
                    dict(key, readout_key=readout_key, neuron_id=u, unit_score=c)
                    for u, c in zip(count(), perf_scores.pearson)
                ]
            )

        for unit_key in unit_scores:
            unit_key["oracle_score"] = (oracle_units & unit_key).fetch1("pearson")
            unit_key["fraction_oracle"] = (
                unit_key["unit_score"] / unit_key["oracle_score"]
            )
    ...
    def make(self, key):
        ...
        for readout_key, testloader in testloaders.items():
            ...
            y, y_hat = compute_predictions(
                testloader, comb_model, readout_key, eye_pos=mu_eye, behavior=mu_beh
            )
            perf_scores = compute_scores(y, y_hat)
...
class MEI(dj.Computed):
    ...
    def key_source(self):
        ...
        return (
            TargetModel() * MEIParameter() * TargetDataset.Unit
    ...
    def make(self, key):
        ...
        gen_image = np.random.normal(
            background_color, 8, (original_h, original_w, channels)
        )
        ...
        with torch.no_grad():
            img = torch.Tensor(process(gen_image, mu=bias, sigma=scale)[None, ...]).to(
                "cuda"
            )
...
class GaussianMask(dj.Computed):
    ...
    def make(self, key):
        ...
        *_, mask = fit_gauss_envelope(img)
        key["mei_mask"] = (
            mask ** factor
        )  # this helps to counteract the overly wide Gaussian fit
        self.insert1(key)
...
class MEIMask(dj.Computed):
    ...
    def make(self, key):
        delta_thr, size_thr, expansion_sigma, expansion_thr, filter_sigma = (
            MaskParameter & key
        ).fetch1(
            "delta_thr", "size_thr", "expansion_sigma", "expansion_thr", "filter_sigma"
        )
        ...
        mask = (
            gaussian_filter(mask.astype(float), sigma=expansion_sigma) > expansion_thr
        )
...
class ProcessedImagenet(dj.Computed):
    ...
    key_source = (
        imagenet.Album.Single * Preprocessing & "preproc_id = 0 and collection_id = 4"
    )
...
class TightMEIMask(dj.Computed):
    ...
    def make(self, key):
        ...
        stdev_size_thr, filter_sigma, target_reduction_ratio = (
            TightMaskParameter & key
        ).fetch1("stdev_size_thr", "filter_sigma", "target_reduction_ratio")
        ...
        def get_activation(mei):
            with torch.no_grad():
                img = torch.Tensor(
                    process(mei[..., None], mu=bias, sigma=scale)[None, ...]
                ).to("cuda")
                activation = adj_model(img).data.cpu().numpy()[0]
            return activation
...
class BestImageNet(dj.Computed):
    ...
    def make(self, key):
        ...
        _, img_shape, bias, mu_beh, mu_eye, scale = prepare_data(key, readout_key)
        target_scan = (StaticMultiDataset.Member & key).fetch1(
            "KEY"
        )  # note that this only works for networks trained on a single scan StaticMultiDataset
        images, image_ids = (
            Frame * ConditionTier * stimulus.Frame
            & target_scan
            & 'image_class="imagenet"'
        ).fetch("frame", "image_id")
        ...
        for image in tqdm(images):
            ...
            image = torch.tensor(
                process(image, mu=bias, sigma=scale)[None, ...],
                dtype=torch.float32,
                requires_grad=True,
                device="cuda",
            )
...
class BestMaskedImageNet(dj.Computed):
    ...
    def __init__(self, *args, **kwargs):
        ...
        warnings.warn(
            "Use of this table is deprecated. It is kept only for record keeping purpose"
        )
...
class BestGaussianMaskedImageNet(dj.Computed):
    ...
    def make(self, key):
        ...
        for image in tqdm(
            images
        ):  # by the way, this approach is not too efficient -- better would have been to batch images
            # apply the mask to the image
            image = np.atleast_3d(
                mask_image(image, mask, bias)
            )  # ensure channel dimension exist
            # prepare as tensor
            image = torch.tensor(
                process(image, mu=bias, sigma=scale)[None, ...],
                dtype=torch.float32,
...
class GaussianMaskedImageResponse(dj.Computed):
    ...
    def make(self, key):
        ...
        image = (
            ...
            * stimulus.StaticImage.Image
            * StaticMultiDataset.Member
            & (BestGaussianMaskedImageNet & key)
        ).fetch1("frame")
        ...
        def evaluate_image(x):
            x = np.atleast_3d(x)
            x = torch.tensor(
                process(x, mu=bias, sigma=scale)[None, ...],
                dtype=torch.float32,
                requires_grad=False,
                device="cuda",
            )
...
class GaussianMaskedSelectedImage(dj.Computed):
    ...
    def make(self, key):
        ...
        image = (
            (stimulus.StaticImage.Image & (BestGaussianMaskedImageNet & key))
            .fetch1("image")
            .astype("float")
        )
...
class BestTightMaskedImageNet(dj.Computed):
    ...
    def make(self, key):
        ...
        images, image_ids = (
            ProcessedImagenet & target_scan & "collection_id = 4"
        ).fetch("frame", "image_id")
...
class MEIActivation(dj.Computed):
    ...
    def make(self, key):
        ...
        target_mean, target_contrast, force_stats = (ImageConfig() & key).fetch1(
            "img_mean", "img_contrast", "force_stats"
        )
        mei, clipped, actual_contrast = adjust_contrast(
            mei, target_contrast, mu=target_mean, force=force_stats
        )
...
class JitterInPlace(dj.Computed):
    ...
    def make(self, key):
        ...
        neuron_id = key["neuron_id"]
        print(
            "Jitter analysis: Working on neuron_id={}, readout_key={}".format(
                neuron_id, readout_key
            )
        )
...
class RFJitterInPlace(dj.Computed):
    ...
    def make(self, key):
        ...
        rf, clipped, actual_contrast = adjust_contrast(
            rf, target_contrast, mu=target_mean, force=force_stats
        )
        ...
        with torch.no_grad():
            img = torch.Tensor(
                process(rf[..., None], mu=bias, sigma=scale)[None, ...]
            ).to("cuda")
...
class ShiftedRF(dj.Computed):
    ...
    def make(self, key):
        ...
        with torch.no_grad():
            img = torch.Tensor(
                process(shifted_rf[..., None], mu=bias, sigma=scale)[None, ...]
            ).to("cuda")
...
class ShiftedMEI(dj.Computed):
    ...
    def make(self, key):
        ...
        with torch.no_grad():
            img = torch.Tensor(
                process(shifted_mei[..., None], mu=bias, sigma=scale)[None, ...]
            ).to("cuda")
            activations = adj_model(img).data.cpu().numpy()[0]
...
class ProcessedMaskedMEI(dj.Computed):
    ...
    def make(self, key):
        ...
        target_mean, target_contrast, zoom_factor, force_stats = (
            ProcessedImageConfig() & key
        ).fetch1("img_mean", "img_contrast", "zoom_factor", "force_stats")
        ...
        # adjust the contrast
        lum_masked_mei, clipped, actual_contrast = adjust_contrast_with_mask(
            lum_mei,
            mask,
        ...
        small_masked_mei = cv2.resize(
            masked_mei, original_shape, interpolation=cv2.INTER_AREA
        ).astype(np.float32)
        ...
        with torch.no_grad():
            img = torch.Tensor(
                process(small_masked_mei[..., None], mu=bias, sigma=scale)[None, ...]
            ).to("cuda")
            activations = adj_model(img).data.cpu().numpy()[0]
...
class ProcessedTightMaskedMEI(dj.Computed):
    ...
    def make(self, key, verbose=False):
        ...
        lum_masked_mei, _ = adjust_img_stats(
            lum_mei,
            mu=target_mean,
            sigma=target_contrast,
            mask=mask,
            img_min=lum_min,
            img_max=lum_max,
            verbose=verbose,
...
class ProcessedTightMaskedImage(dj.Computed):
    ...
    def make(self, key, verbose=False):
        ...
        img = (
            (stimulus.StaticImage.Image & (BestTightMaskedImageNet & key))
            .fetch1("image")
            .astype("float")
        )
        img = np.clip(img, 0, 255)
        ...
        # adjust the contrast
        lum_masked_img, _ = adjust_img_stats(
            lum_img,
            mu=target_mean,
            sigma=target_contrast,
            mask=mask,
        ...
        small_masked_img = cv2.resize(
            masked_img, original_shape, interpolation=cv2.INTER_AREA
        ).astype(np.float32)
        ...
        with torch.no_grad():
            img = torch.Tensor(
                process(small_masked_img[..., None], mu=bias, sigma=scale)[None, ...]
            ).to("cuda")
            activations = adj_model(img).data.cpu().numpy()[0]
...
class ProcessedTightUnmaskedImage(dj.Computed):
    ...
    def make(self, key, verbose=False):
        ...
        # adjust the contrast
        _, lum_unmasked_img = adjust_img_stats(
            lum_img,
            mu=target_mean,
            sigma=target_contrast,
            mask=mask,
        ...
        small_unmasked_img = cv2.resize(
            unmasked_img, original_shape, interpolation=cv2.INTER_AREA
        ).astype(np.float32)
        ...
        with torch.no_grad():
            img = torch.Tensor(
                process(small_unmasked_img[..., None], mu=bias, sigma=scale)[None, ...]
            ).to("cuda")
            activations = adj_model(img).data.cpu().numpy()[0]
...
class ProcessedMaskedImage(dj.Computed):
    ...
    def make(self, key):
        ...
        img = (
            (stimulus.StaticImage.Image & (BestMaskedImageNet & key))
            .fetch1("image")
            .astype("float")
        )
        img = np.clip(img, 0, 255)
        ...
        # adjust the contrast
        lum_masked_img, clipped, actual_contrast = adjust_contrast_with_mask(
            lum_img,
            mask,
            contrast=target_contrast,
            mu=target_mean,
...
class ProcessedShiftedMEI(dj.Computed):
    ...
    def make(self, key):
        ...
        # adjust the contrast
        lum_mei, clipped, actual_contrast = adjust_contrast(
            lum_mei,
            target_contrast,
            mu=target_mean,
            img_min=lum_min,
        ...
        # shift the image
        x_shift, y_shift = int(zoom_factor * key["x_shift"]), int(
            zoom_factor * key["y_shift"]
        )
        shifted_mei = np.roll(np.roll(mei, x_shift, 1), y_shift, 0)

        small_shifted = cv2.resize(
            shifted_mei, original_shape, interpolation=cv2.INTER_AREA
        ).astype(np.float32)
        ...
        with torch.no_grad():
            img = torch.Tensor(
                process(small_shifted[..., None], mu=bias, sigma=scale)[None, ...]
            ).to("cuda")
            activations = adj_model(img).data.cpu().numpy()[0]
...
class MEIConfusion(dj.Computed):
    ...
    model_map = {"cnn": ModelGroup.CNNModel, "linear": ModelGroup.LinearModel}
    ...
    def key_source(self):
        ...
        return (
            units.proj(src_neuron_id="neuron_id")
            * MEIParameter
            * ImageConfig
            * ModelType
            & MEI.proj(src_neuron_id="neuron_id")
            & mei_param
        )

    def make(self, key):
        target_neurons = (
            TargetDataset.Unit & (CorrectedHighUnitSelection & "hu_rank < 150") & key
        ).fetch("neuron_id")
        ...
        keys = [
            dict(neuron_id=nid, confusion_activation=activation, **key)
            for nid, activation in zip(target_neurons, activations)
        ]
        self.insert(keys)
...
class WrongMEIConfusion(dj.Computed):
    ...
    def key_source(self):
        ...
        return (
            ...
            * ImageConfig
            * ModelType
            & WrongMEI.proj(src_neuron_id="neuron_id")
            & mei_param
        )
...
class CrossShiftedMEIActivation(dj.Computed):
    ...
    def make(self, key):
        ...
        lin_key = (
            ModelGroup.LinearModel & (ModelGroup & (ModelGroup.CNNModel & key))
        ).fetch1("KEY")
...
class SpotCenteredMEI(dj.Computed):
    ...
    def key_source(self):
        # restriction by CorePlusReadout is needed to link dataconfig with the appropriate model
        return (
            TargetModel() * MEIParameter() * ImageConfig() * TargetDataset.Unit
            & NetworkConfig.CorePlusReadout
        )
...
class GaborRange(dj.Lookup):
    ...
    contents = [
        [
            ...
            [0, 90, 180, 270],
            [4, 7, 10, 15, 20],
            np.linspace(0, 180, 8, endpoint=False),
            [2, 3, 5, 7, 9],
...
class GaborDataset(Dataset):
    ...
    def __init__(
        ...
        self.args = list(
            product(
                [height], [width], phases, wavelengths, orientations, sigmas, dys, dxs
            )
        )
    ...
    def __getitem__(self, item):
        ...
        gabor, _ = adjust_img_stats(
            gabor,
            mu=self.target_mean,
            sigma=self.target_contrast,
            img_min=self.f_min,
            img_max=self.f_max,
        )
...
class BestGabor(dj.Computed):
    ...
    def key_source(self):
        # restriction by CorePlusReadout is needed to link dataconfig with the appropriate model
        return (
            TargetModel * ProcessedImageConfig * GaborRange * TargetDataset
            & NetworkConfig.CorePlusReadout
        )
    ...
    def make(self, key):
        # Get gabor dataset
        gabor_params = (GaborRange & key).fetch1(
            "height",
            "width",
            "phases",
            "wavelengths",
        ...
        contrast_params = (ProcessedImageConfig & key).fetch1(
            "img_mean", "img_contrast", "force_stats"
        )
        ...
        readout_keys = (dj.U("readout_key") & (TargetDataset.Unit & key)).fetch(
            "readout_key"
        )
        for readout_key in readout_keys:
            ...
            adj_model = get_adj_model(
                models, readout_key, neuron_id=slice(None), mu_eye=mu_eye  # all cells
            )
            ...
            if activations.shape[1] != len(
                TargetDataset.Unit & key & {"readout_key": readout_key}
            ):
...
class ProcessedBestGabor(dj.Computed):
    ...
    def make(self, key):
        ...
        small_gabor = cv2.resize(
            gabor, original_shape, interpolation=cv2.INTER_AREA
        ).astype(np.float32)
        ...
        with torch.no_grad():
            img = torch.Tensor(
                process(small_gabor[..., None], mu=bias, sigma=scale)[None, ...]
            ).to("cuda")
            activations = adj_model(img).data.cpu().numpy()[0]
...
class GaborLimits(dj.Lookup):
    ...
    """
    contents = [
        [1, 36, 64, 0, 360, 4, 20, 0, 180, 2, 9, -0.35, 0.35, -0.35, 0.35],
    ]
...
class OptimalGabor(dj.Computed):
    ...
    def make(self, key):
        ...
        # Get model
        _, img_shape, bias, mu_beh, mu_eye, scale = prepare_data(
            key, key["readout_key"]
        )
        models = get_multi_model(key)
        adj_model = get_adj_model(
            models, key["readout_key"], neuron_id=key["neuron_id"], mu_eye=mu_eye
        )

        # Get optimization bounds per parameter
        params = (GaborLimits & key).fetch1()
        bounds = [
            (params["lower_phase"], params["upper_phase"]),
        ...
        for seed in tqdm([1, 12, 123, 1234, 12345]):  # try 5 diff random seeds
            ...
            res = optimize.minimize(
                neg_model_activation, x0=res.x, method="Nelder-Mead"
            )
