...
#     else:
#         raise ValueError(
#             "Packing transform type {} is not supported yet".format(
...
#     configs = dict(
#         filename=filename,
#         data_keys=ds.data_keys,
#         cache_raw=ds.cache_raw,
#         stats_source=ds.stats_source,
#     )
...
def trunc_center_forward(self, x, shift=None):
    ...
    for i, xx in enumerate(self.gauss_pyramid(x)):
        if i == self.scale_idx:
            ...
            ctr_h, ctr_w = img_h // 2, img_w // 2
            pools.append(
                xx[..., ctr_h, ctr_w].unsqueeze(-1).expand(N, ch, self.outdims)
            )
...
def discretized_forward(self, x, shift=None):
    ...
    for xx in self.gauss_pyramid(x):
        ...
        adj_grid = (
            torch.round((grid + 1) / 2 * (img_shape - 1)) * 2 / (img_shape - 1) - 1
        )
...
def all_forward(self, x, shift=None):
    ...
    m = self.gauss_pyramid.scale_n + 1
    feat = (
        self.features.view(m * c, self.outdims, 1, 1).permute([1, 0, 2, 3]).contiguous()
    )
...
def trunc_center_readout(scale_idx=0):
    print(
        "Using truncated centered forward with scale={} on SpatialTransformerPyramid2d".format(
            scale_idx
        )
    )
    SpatialTransformerPyramid2d.old_forward = SpatialTransformerPyramid2d.forward
...
def fft_smooth(grad, factor=1 / 4):
    ...
    # grad_fft = tf.fft2d(tf.cast(grad, tf.complex64))
    tw = np.minimum(
        np.arange(0, w), np.arange(w - 1, -1, -1), dtype=np.float32
    )  # [-(w+2)//2:]
    ...
    pp = torch.rfft(grad.data, 2, onesided=False) #old version of torch.fft.rfft2
    return torch.irfft(pp * F, 2, onesided=False) #old version of torch.fft.irfft2
...
def make_step(
    ...
    if train_norm is not None and train_norm > 0.0:
        ...
        if img_idx.any():
            img = (
                src.clone()
            )  # avoids overwriting original image but lets gradient through
            img[img_idx] = (
                src[img_idx] / (batch_std(src[img_idx], keepdim=True) + _eps)
            ) * (train_norm / scale)
    ...
    src.data += (
        (step_size / (torch.abs(grad.data).mean() + eps)) # eps for not deviding by 0 
        * (step_gain / 255)
        * grad.data
    )
    ...
    if norm is not None and norm > 0.0:
        ...
        src.data[data_idx] = (
            src.data / (batch_std(src.data, keepdim=True) + _eps) * norm / scale
        )[data_idx]
...
def deepdraw(
    ...
    **step_params
    ...
    ###########################################################################
    ##################Gaussian Blurring Factor#################################
    ###########################################################################
            
            
            sigma = (
                o["start_sigma"] # s_0
                + ((o["end_sigma"] - o["start_sigma"]) # delta_s
                * i) # iteration counter
                / o["iter_n"] # total number of iterations
            )


    ###########################################################################
    ########################EQUIVALENT TO LEARINGING RATE######################
    ###########################################################################
...
def diverse_deepdraw(
    ...
    **step_params
    ...
    if len(base_img) < 2:
        raise ValueError(
            "You need to pass at least two initial images. Did you mean to "
            "use deepdraw()?"
        )
        ...
        for i in range(o["iter_n"]):
            if imw > w:
                if random_crop:
                    ...
                    # insert the crop into src.data[0]
                    src.data[:].copy_(
                        torch.Tensor(image[..., ox : ox + w, oy : oy + h])
                    )
                else:
                    ox = int((imw - w) / 2)
                    oy = int((imh - h) / 2)
            ...
            if div_weight > 0:
                ...
                if div_metric == "correlation":
                    ...
                    distance_matrix = -(
                        torch.mm(residuals, residuals.t())
                        / torch.sqrt(torch.ger(ssr, ssr))
                        + 1e-12
                    )
                elif div_metric == "cosine":
                    image_norms = torch.norm(images, dim=-1)
                    distance_matrix = -(
                        torch.mm(images, images.t())
                        / (torch.ger(image_norms, image_norms) + 1e-12)
                    )
                elif div_metric == "euclidean":
                    distance_matrix = torch.norm(
                        images.unsqueeze(0) - images.unsqueeze(1), dim=-1
                    )
                else:
                    raise ValueError(
                        "Invalid distance metric {} for the diversity term".format(
                            div_metric
                        )
                    )
                ...
                triu_idx = (
                    torch.triu(
                        torch.ones(len(distance_matrix), len(distance_matrix)),
                        diagonal=1,
                    )
                    == 1
                )
                if div_linkage == "minimum":
                    ...
                    raise ValueError(
                        "Invalid linkage for the diversity term: {}".format(div_linkage)
                    )
...
def create_gabor(
    height=36, width=64, phase=0, wavelength=10, orientation=0, sigma=5, dy=0, dx=0
):
    ...
    if abs(dx) > 1 or abs(dy) > 1:
        raise ValueError(
            "Please express translations as factors of the height/width,"
...
def plot_images(
    ...
    n_rows=15,
    order_by="pearson",
    panels=("normed_rf", "normed_mei"),
    panel_names=("RF", "MEI"),
    cmaps=("coolwarm", "gray"),
    y_infos=("{prefix}test_corr", "pearson"),
    save_path=None,
):
    if names is None:
        names = prefixes
    ...
    with sns.axes_style("white"):
        ...
        st = fig.suptitle(
            "MEIs on Shuffled {} dataset: {}".format(brain_area, ", ".join(names))
        )
    ...
    for ax_row, (_, data_row), row_index in zip(axs, area_data.iterrows(), count()):
        for ax_group, prefix, name in zip(grouper(n_panels, ax_row), prefixes, names):
            for ax, panel, panel_name, y_info, cm in zip(
                ax_group, panels, panel_names, y_infos, cmaps
            ):
                if row_index == 0:
                ...
                if y_info is not None:
                    ax.set_ylabel(
                        "{:0.2f}%".format(data_row[y_info.format(prefix=prefix)] * 100)
                    )
...
def tile_images(images, rows, cols, vpad=0, hpad=0, normalize=False, base=0):
    ...
    total_image = (
        np.zeros((h + (h + vpad) * (rows - 1), w + (w + hpad) * (cols - 1))) + base
    )
